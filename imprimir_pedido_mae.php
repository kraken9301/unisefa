...
// Obtener datos del pedido y maestro
$queryPedido = "SELECT p.id, p.fecha_solicitud, m.nombre, m.apellido, m.ci, m.especialidad, m.fecha_ingreso
                FROM pedidos p
...
$nombreCompleto = utf8_decode($pedido['nombre'] . ' ' . $pedido['apellido']);
...
$pdf->Cell(0, 10, 'Resumen del Pedido - Maestro', 0, 1, 'C');
$pdf->Ln(5);

// Datos del maestro
$pdf->SetFont('Arial', '', 12);
$pdf->Cell(0, 8, 'Nombre: ' . $nombreCompleto, 0, 1);
$pdf->Cell(0, 8, 'CI: ' . utf8_decode($pedido['ci']), 0, 1);
$pdf->Cell(0, 8, 'Especialidad: ' . utf8_decode($pedido['especialidad']), 0, 1);
$pdf->Cell(0, 8, 'Fecha de Ingreso: ' . date("d/m/Y", strtotime($pedido['fecha_ingreso'])), 0, 1);
$pdf->Cell(0, 8, 'Fecha del Pedido: ' . date("d/m/Y", strtotime($pedido['fecha_solicitud'])), 0, 1);
$pdf->Ln(5);
...
// Salida del PDF
$pdf->Output('I', 'pedido_maestro_' . $idPedido . '.pdf');
?>
