...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Verificar si el estudiante ya tiene un pedido
    $stmtPedido = $conn->prepare("SELECT id FROM pedidos WHERE id_estudiante = ?");
    ...
    if ($resultPedido->num_rows > 0) {
        ...
        // Crear pedido
        $fecha = date('Y-m-d');
        $stmtCrearPedido = $conn->prepare("INSERT INTO pedidos (id_estudiante, fecha_solicitud) VALUES (?, ?)");
        $stmtCrearPedido->bind_param("is", $idEstudiante, $fecha);
